...
for i in tqdm(
    range(len(canopus_labels)), desc="Converting CANOPUS SMILES to InChI", leave=False
):
...
canopus_train_df.to_csv(
    "../data/fp2mol/canopus/preprocessed/canopus_train.csv", index=False
)

canopus_test_df = pd.DataFrame(canopus_test_inchis, columns=["inchi"])
canopus_test_df.to_csv(
    "../data/fp2mol/canopus/preprocessed/canopus_test.csv", index=False
)

canopus_val_df = pd.DataFrame(canopus_val_inchis, columns=["inchi"])
canopus_val_df.to_csv(
    "../data/fp2mol/canopus/preprocessed/canopus_val.csv", index=False
)
...
for i in tqdm(
    range(len(msg_labels)), desc="Converting MSG SMILES to InChI", leave=False
):
...
hmdb_train_inchis = [
    inchi for inchi in hmdb_train_inchis if inchi not in excluded_inchis
]
...
coconut_train_inchis = [
    inchi for inchi in coconut_train_inchis if inchi not in excluded_inchis
]

coconut_train_df = pd.DataFrame(coconut_train_inchis, columns=["inchi"])
coconut_train_df.to_csv(
    "../data/fp2mol/coconut/preprocessed/coconut_train.csv", index=False
)

coconut_val_df = pd.DataFrame(coconut_val_inchis, columns=["inchi"])
coconut_val_df.to_csv(
    "../data/fp2mol/coconut/preprocessed/coconut_val.csv", index=False
)
...
combined_train_inchis = [
    inchi for inchi in combined_train_inchis if inchi not in excluded_inchis
]

combined_train_df = pd.DataFrame(combined_train_inchis, columns=["inchi"])
combined_train_df.to_csv(
    "../data/fp2mol/combined/preprocessed/combined_train.csv", index=False
)

combined_val_df = pd.DataFrame(combined_val_inchis, columns=["inchi"])
combined_val_df.to_csv(
    "../data/fp2mol/combined/preprocessed/combined_val.csv", index=False
)
