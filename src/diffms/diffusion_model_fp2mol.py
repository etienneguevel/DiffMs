...
from diffms.models.transformer_model import (
    GraphTransformer,
    GraphTransformerV2,
    GraphTransformerV3,
    GraphTransformerV4,
)
from diffms.diffusion.noise_schedule import (
...
from diffms.metrics.diffms_metrics import (
    K_ACC_Collection,
...
class FP2MolDenoisingDiffusion(pl.LightningModule):
    def __init__(
        ...
        logging.info(
            f"Finished initializing FP2MolDenoisingDiffusion on GPU {self.device}"
        )
    ...
    def on_train_epoch_end(self) -> None:
        ...
        if self.global_rank == 0:
            logging.info(
                f"Epoch {self.current_epoch}: X_CE: {to_log['train_epoch/x_CE']:.2f} -- E_CE: {to_log['train_epoch/E_CE']:.2f} -- time: {to_log['train_epoch/time']:.2f}"
            )
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.global_rank == 0:
            ...
            logging.info(
                f"Val NLL: {val_nll:.4f} \t Best Val NLL:  {self.best_val_nll}"
            )

            # save self.model to models/graph_transformer_{epoch}.pt
            torch.save(
                self.model.state_dict(),
                f"models/graph_transformer_{self.current_epoch}.pt",
            )
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if self.global_rank == 0:
            logging.info(
                f"Epoch {self.current_epoch}: Test NLL {metrics[0]:.2f} -- Test Atom type KL {metrics[1]:.2f} -- Test Edge type KL: {metrics[2]:.2f} -- Test Edge type logp: {metrics[3]:.2f} -- Test Edge type CE: {metrics[5]:.2f}"
            )
    ...
    def sample_batch(self, batch: Batch) -> list[Chem.Mol]:
        ...
        for s_int in tqdm(reversed(range(0, self.T)), desc="Sampling", leave=False):
            s_array = s_int * torch.ones(
                (len(batch), 1), dtype=torch.float32, device=self.device
            )
            t_array = s_array + 1
